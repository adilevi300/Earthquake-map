...
map.getView().fit([-19268757.188920, -11745613.086059, 19811837.612282, 15586704.159848], map.getSize());
...
function createPopupField(currentFeature, currentFeatureKeys, layer) {
    // Keep the exported popup fields in a clear academic reading order.
    var preferredOrder = layer.get('popuplayertitle') === 'Contextual Factors'
        ? ['Country', 'Year_Web', 'Governance', 'Pverty_line', 'Stractural vulnerbility', 'Socioeconomic_Vulnerability']
        : ['Country', 'Year_Web', 'Magnitude', 'Death', 'Injured', 'People_affected', 'Damage_USD'];
    currentFeatureKeys = preferredOrder.concat(currentFeatureKeys.filter(function(k) { return preferredOrder.indexOf(k) === -1; }));
    var popupText = '';
    for (var i = 0; i < currentFeatureKeys.length; i++) {
        if (currentFeatureKeys[i] != 'geometry' &&
            ...
            if (layer.get('fieldLabels')[currentFeatureKeys[i]] == "hidden field") {
                ...
                if (currentFeature.get(currentFeatureKeys[i]) == null || currentFeature.get(currentFeatureKeys[i]) === '') {
                    continue;
                }
            }
        ...
        if (typeof clusteredFeatures !== "undefined") {
            if (doPopup) {
                for(var n=0; n<clusteredFeatures.length; n++) {
                    ...
                    popupText += '<li><table>'
                    popupText += '<caption class="popup-layer-title">' + layer.get('popuplayertitle') + '</caption>';
...
function updatePopup() {
    if (popupContent) {
        ...
        container.style.background = '#24527A';
        container.style.backgroundColor = '#24527A';
        container.style.borderColor = '#6FA3CC';
...
function onSingleClickFeatures(evt) {
    ...
    map.forEachFeatureAtPixel(pixel, function(feature, layer) {
        if (layer && feature instanceof ol.Feature && (layer.get("interactive") || layer.get("interactive") === undefined)) {
            ...
            // Stop after the topmost interactive feature so overlapping layers
            // open separate popups instead of being combined.
            return true;
        }
    });
    if (popupText === '<ul>') {
...
// Final layer selector: lets the reader independently show/hide both analytical layers.
var finalLayerSwitcher = new LayerSwitcher({
    activationMode: 'click',
    startActive: false,
    tipLabel: 'Layers',
    collapseTipLabel: 'Close layers',
    groupSelectStyle: 'children'
});
map.addControl(finalLayerSwitcher);
